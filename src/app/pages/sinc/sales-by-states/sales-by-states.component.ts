...
export class SalesByStatesComponent implements OnInit {
  ...
  executarMapa(data: any): void {
    ...
    this.chartInstance.showLoading(loading);
          ...
          series: [
            {
              ...
              label: {
                ...
                formatter(params: any) {
                  ...
                  const qtyNumber = (typeof qty === 'number' || qty === 'string') ? parseFloat(qty.toString()) : 0;

                  const qtyItemsNumber = (typeof qtyItems === 'number' || typeof qtyItems === 'string') ? parseFloat(qtyItems.toString()) : 0;
                  ...
                  if (valueFormated !== undefined && valueFormated != null) {
                    ...
                    const qtyFormated = qtyNumber?.toLocaleString('pt-BR');
                    const qtyItemsFormated = qtyItemsNumber?.toLocaleString('pt-BR');
                    const formattedLabel = `{c|${params.name}}\n{c|${valueFormatted}}\n{z|Vendas: ${qtyFormated}}\n{z|Qtd Itens: ${qtyItemsFormated}}`;
